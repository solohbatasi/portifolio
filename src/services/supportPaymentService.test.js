...
describe('supportPaymentService', () => {
  it('posts only the expected payment payload to the portfolio backend', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ transaction_id: 'txn_safe', status: 'pending' }),
    })
    ...
    expect(fetchMock.mock.calls[0][0]).toBe('/api/support/coffee')
  ...
  it('checks status through the portfolio backend and never calls Kadi directly', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ status: 'success' }) })
    vi.stubGlobal('fetch', fetchMock)

    await fetchCoffeePaymentStatus('txn_123')

    expect(fetchMock.mock.calls[0][0]).toBe('/api/support/coffee/txn_123')
